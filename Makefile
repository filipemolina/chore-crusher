...
LDFLAGS := -X github.com/filipemolina/chore-completer/src/constants.version=$(VERSION)

dev:
	go run main.go

# Build and install to $(go env GOPATH)/bin (~/go/bin by default).
# ~/go/bin is on PATH, so `complete` is runnable immediately
# after `make build` — no sudo, no extra setup.
build:
	go build -ldflags "$(LDFLAGS)" -o "$(shell go env GOPATH)/bin/complete" .
