...
farol has two top-level pages — Active and Archived — switched with `1`/`2`, rendered as digit tabs in the header the way `../cais`'s mainmenu does. `1` and `2` work from anywhere no text input owns the keyboard, including from inside the other page.

**The Archive page (`2`) is a full-body takeover, not a modal — a third thing, distinct from both of the above.** Unlike Details it does not layer a box over the body: while it is open, `renderBody()` returns the Archive page's own `View` in place of the Tasks/Lists split entirely, the same "one surface replaces the whole row" shape Tasks-alone already has with Lists hidden. That is why it does not make focus "three targets" or the body "two side surfaces": it is never in the `tab` cycle (entered/left by explicit `2`/`1`/close transitions, exactly like Details), and it does not share the row with Tasks the way Lists does — it replaces the row outright. It owns the keyboard the same way Details does — AppModel intercepts every keypress ahead of the normal dispatch, right after the Details check — so only its own bindings (`keys.ArchivePage`) plus `esc` and `1` are live while it is open, and the footer goes blank the same way it does for Details, with the page rendering its own hint line instead.
