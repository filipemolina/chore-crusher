...
- **Search that spans every list.** `/` filters the current list live, keeping ancestor chains visible. `F` opens a picker over every list at once, ranking title matches above notes-only hits.
- **Fourteen themes, previewed live.** Four farol themes and ten community palettes (Catppuccin, Gruvbox, Tokyo Night, Nord, Dracula, Solarized, One Dark, Everforest, Rosé Pine, Kanagawa). `T` opens the picker; cursor movement previews live; `Enter` persists.
