...
// (store.DerivedProgress). Err holds a read failure; the other fields are
// zero when Err is set. The panel replaces its displayed fields from this
// only while its editor is clean — a dirty draft keeps its edits (docs/DESIGN.md
// §5, the poll-refresh-while-clean rule).
...
// RefreshDetails reads one task's details and derived progress. It never
// writes, and it converts the store row with apptypes.FromStore so no
// store.Task crosses into a component (the same apptypes boundary the other
// refresh commands keep, docs/DESIGN.md §10).
