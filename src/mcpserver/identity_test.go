...
// TestServerIdentityIsUniquePerProcess pins the tag-generation contract:
// FAROL_AGENT wins verbatim, and when it is unset the tag is unique per
// resolution rather than the constant "agent" it used to be. The constant
// is what made two unconfigured clients compare equal and write over each
// other; see TestTwoUnconfiguredSessionsCannotWriteEachOthersTasks.
//
...
func TestServerIdentityIsUniquePerProcess(t *testing.T) {
	...
	if first == second {
		t.Fatalf("serverIdentity returned %q twice with FAROL_AGENT unset; "+
	...
	if got := serverIdentity(); got != "pi" {
		t.Fatalf("serverIdentity with FAROL_AGENT=pi = %q, want pi", got)
	}
}
