...
// TestFromStoreConvertersCopyEveryField guards the boundary that
// docs/plan/mcp-assignment-and-priorities.md §6.4 names as a silent failure
// mode: the store -> apptypes conversions are functions rather than type
// aliases so a field cannot leak between the layers, but nothing made that
// promise enforceable. A field added to a store struct AND to its apptypes
